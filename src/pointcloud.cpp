...
void lcloud::getMaskAccordingToColor(const Mat &cv_rgbimg, Mat &mask)
{
  ...
  inRange(hsvImage, Scalar(0, 0, 221), Scalar(180, 30, 255), mask);
}
...
void lcloud::getPLY()
{
  ...
  while (source_downsampled->points.size() > 0.1 * nr_points)
  {
    ...
    writer.write<PointXYZ>("/home/ddxy/Downloads/pointcloud/kinect/camera1/testcloud/SPHERE" + to_string(i) + ".ply", *sphere, false);
